...
# Thêm root vào path để import (đã điều chỉnh đường dẫn vì file nằm trong src/datasets/)
...
def denoise_skeleton(skel_25):
    ...
    2. Lọc theo độ phủ (Spread Ratio: X-spread / Y-spread > 0.8)
    ...
    for b in range(num_bodies):
        ...
        # 2. Lọc theo Spread Ratio (Lọc từng frame theo chuẩn MAMP/ST-GCN)
        # Tránh việc loại bỏ nhầm người thật khi họ di chuyển ngang (làm x_spread tổng thể tăng cao)
        valid_frames_data = body[non_zero_mask]
        if len(valid_frames_data) == 0: continue
        
        # Tính độ rộng x, y cho TỪNG frame [T_valid]
        dx = valid_frames_data[:, :, 0].max(axis=1) - valid_frames_data[:, :, 0].min(axis=1)
        dy = valid_frames_data[:, :, 1].max(axis=1) - valid_frames_data[:, :, 1].min(axis=1)
        
        # Frame hợp lệ là frame có hình dáng người chuẩn (ngang <= 0.8 dọc)
        is_valid_frame = (dx <= 0.8 * (dy + 1e-8))
        valid_ratio = np.mean(is_valid_frame)
        
        # Nếu hơn 70% số frame là nhiễu hình học (bàn ghế) thì mới loại bỏ
        if valid_ratio < 0.3:
            continue
            
        # 3. Tính độ biến thiên chuyển động (Motion Variance)
        # LỖI CŨ: Tính trên `body` gốc chứa cả frame 0 -> np.diff tạo ra vận tốc ảo cực lớn ở ranh giới.
        # SỬA LẠI: Chỉ tính trên các frame đã sạch (không chứa số 0, không bị bành trướng)
        clean_frames = valid_frames_data[is_valid_frame]
        if len(clean_frames) <= 1: 
            continue
            
        motion = np.diff(clean_frames, axis=0) # [T_clean-1, 25, 3]
...
def main():
    ...
    DATA_PATHS = [
        "./DATA/nturgbd_skeletons_s001_to_s017/nturgb+d_skeletons",
        "./DATA/nturgbd_skeletons_s018_to_s032"
    ]
