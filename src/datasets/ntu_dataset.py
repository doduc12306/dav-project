...
def read_ntu_skeleton(file_path):
    ...
    # BƯỚC 1: Đọc toàn bộ file một cách linh hoạt (Dynamic)
    current_line = 1
    all_frames_raw = []
    max_bodies_in_file = 0
    ...
    for _ in range(num_frames):
        ...
        max_bodies_in_file = max(max_bodies_in_file, num_bodies)
        
        if num_bodies == 0:
            # Nếu frame trống, copy frame trước đó hoặc mảng rỗng
            all_frames_raw.append(all_frames_raw[-1] if all_frames_raw else [])
            continue
        ...
        all_frames_raw.append(frame_bodies)
        
    # BƯỚC 2: Đồng bộ hóa kích thước mảng (Zero Padding)
    # Đảm bảo tối thiểu là 2 người để tương thích với cấu trúc mạng chuẩn
    target_bodies = max(2, max_bodies_in_file)
    
    all_frames = []
    for frame_bodies in all_frames_raw:
        # Nếu frame đang rỗng (do lấy từ if num_bodies == 0 ở trên mà không có frame trước)
        if not frame_bodies:
            frame_bodies = []
            
        # Độn thêm người "tàng hình" (Zero Padding) cho đến khi đủ target_bodies
        while len(frame_bodies) < target_bodies:
            frame_bodies.append(np.zeros((25, 3)).tolist())
            
        all_frames.append(np.array(frame_bodies)) # [target_bodies, 25, 3]
...
def _load_files(data_dirs):
    ...
    all_paths = []
    seen_blacklists = set()  # Tránh in thông báo trùng lặp khi nhiều thư mục dùng chung blacklist
    ...
    for d in data_dirs:
        ...
        if blacklist_file_found:
            ...
            # Chỉ in lần đầu tiên tìm thấy file blacklist này
            if blacklist_file_found not in seen_blacklists:
                print(f"[_load_files] Blacklist: {len(blacklist)} mẫu bị loại từ {blacklist_file_found}")
                seen_blacklists.add(blacklist_file_found)
        else:
...
def _load_skeleton(path):
    ...
    if path.endswith('.skeleton'):
        raw = read_ntu_skeleton(path) # [T, 4, 25, 3] (Theo bản cập nhật mới)
        if raw is None: return None
        # FALLBACK: Nếu Dataloader bắt buộc phải đọc trực tiếp file raw (không qua Denoising)
        # thì ta chỉ ép nó về [T, 2, 25, 3] bằng cách bốc đại 2 ID đầu tiên để tránh lỗi shape.
        raw = raw[:, :2, :, :]
...
class SJEPA_UnsupervisedDataset(Dataset):
    ...
    def __init__(self, data_path, max_frames=120, mask_ratio=0.9,
                 protocol=None, num_classes=None, segment_length=4, target_dataset='ntu120', **kwargs):
        ...
        if protocol is not None:
            ...
            print(f"[SJEPA_UnsupervisedDataset] Protocol={protocol.upper()}, "
                  f"target_dataset={self.target_dataset} → chỉ dùng TRAIN split.")
    ...
    def _filter_pretrain(self, all_paths, protocol, num_classes):
        ...
        for path in all_paths:
            ...
            # NẾU target_dataset == 'ntu120': Chặn leakage cho TẤT CẢ các hành động từ 1 đến 120
            # NẾU target_dataset == 'ntu60': Chỉ chặn leakage cho hành động <= 60 (Mẹo hút data NTU120)
            if (self.target_dataset == 'ntu120') or (action_id <= 60):
                if protocol == 'xsub':
            ...
            # ĐIỂM SÁNG: Nếu target_dataset='ntu60' và action_id > 60, code sẽ nhảy qua khối IF trên
